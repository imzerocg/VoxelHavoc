...
class FloatTxt {
    ...
    draw() { 
        ctx.fillStyle='#ff4d4d'; ctx.globalAlpha=this.l/40; 
...
class Part {
    ...
    draw() { ctx.fillStyle=this.c; ctx.globalAlpha=this.l/30; ctx.fillRect(this.x,this.y,3,3); ctx.globalAlpha=1; }
}

// Global function to trigger an explosion of juice
...
class Proj {
    ...
    update() {
        ...
        if(this.tgt && this.tgt.hp>0 && Math.hypot(this.x-this.tgt.x, this.y-this.tgt.y)<(this.tgt.r==='elephant'?45:25)) {
            ...
            if(this.typ==='ice') this.tgt.frz=80; if(this.typ==='fire') { this.tgt.hp-=5; addVFX(this.tgt.x, this.tgt.y, 5); }
