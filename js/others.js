class FloatTxt {
    ...
    draw() { 
        ...
        ctx.globalAlpha = Math.max(0, this.l / 40); 
...
class Part {
    ...
    draw() { 
        ...
        ctx.globalAlpha = Math.max(0, this.l / 30); 
...
class Proj {
    ...
    update() {
        ...
        if(Math.random() > 0.3) {
            particles.push(new Part(this.x, this.y, this.typ === 'cannonball' ? '#555' : '#fff'));
        }
        ...
        if(this.tgt && this.tgt.hp > 0 && Math.hypot(this.x - this.tgt.x, this.y - this.tgt.y) < (this.tgt.r === 'elephant' ? 45 : 25)) {
            this.tgt.hp -= this.dmg; 
            this.tgt.ft = 5;
            addVFX(this.tgt.x, this.tgt.y, this.dmg);
            
            if(this.typ === 'ice') this.tgt.frz = 80; 
            if(this.typ === 'fire') { 
                this.tgt.hp -= 5; 
                addVFX(this.tgt.x, this.tgt.y, 5); 
            }
            
            let pr = this.tgt.r === 'elephant' ? 0.1 : (this.tgt.r === 'cannon' ? 0 : 0.5); 
            if(this.typ === 'cannonball') pr *= 4;
            
            this.tgt.vx += this.vx * pr; 
            this.tgt.vy += this.vy * pr; 
            this.act = false;
        }
    }
    draw() {
        ctx.shadowBlur = 10; 
        ctx.shadowColor = this.typ === 'fire' ? '#ff4500' : (this.typ === 'ice' ? '#00ffff' : 'transparent');
        ctx.fillStyle = this.typ === 'cannonball' ? '#222' : (this.typ === 'fire' ? '#f39c12' : (this.typ === 'ice' ? '#0ff' : '#fff'));
        ctx.beginPath(); 
        ctx.arc(this.x, this.y, this.typ === 'cannonball' ? 8 : 6, 0, Math.PI * 2); 
        ctx.fill(); 
        ctx.shadowBlur = 0;
    }
}
