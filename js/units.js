class Person {
    constructor(x,y,tm,r) {
        ...
        this.mHp = this.hp; this.age = 0; this.task = null; this.tTmr = 0;
        this.hC=false; this.ft=0; this.cd=0; this.pCd=0; this.dT=0;
    ...
    draw() {
        ...
        // FIXED: The flash color uses the 'ft' (flash timer) correctly now
    ...
    update() {
        // FIXED: This line now properly counts down the flash timer (ft)
        if(this.cd>0) this.cd--; if(this.pCd>0) this.pCd--; if(this.dT>0) this.dT--; if(this.ft>0) this.ft--; this.age++;
        ...
        // CLEARED: Removed all Shrine and Weapon-Crafting logic. Only houses remain for basic cloning.
        ...
        if(nE) {
            ...
            if(this.r==='ichigo' && mD<300 && this.cd<=0) { this.kTmr=30; this.cd=150; ppl.forEach(p=>{if(p.t!==this.t && Math.abs(p.y-this.y)<40 && ((Math.cos(a)>0 && p.x>this.x) || (Math.cos(a)<0 && p.x<this.x))){ p.hp-=250; p.ft=5; addVFX(p.x, p.y, 250); }}); }
            if(this.r==='zoro' && mD<60 && this.cd<=0) { this.kTmr=20; this.cd=80; ppl.forEach(p=>{if(p.t!==this.t && Math.hypot(this.x-p.x,this.y-p.y)<60){ p.hp-=150; p.ft=5; addVFX(p.x,p.y,150); p.vx+=Math.cos(a)*10; p.vy+=Math.sin(a)*10; }}); }
            if(this.r==='sukuna' && mD<150 && this.cd<=0) { this.kTmr=40; this.cd=400; shakeTime=10; ppl.forEach(p=>{if(p.t!==this.t && Math.hypot(this.x-p.x,this.y-p.y)<150){ p.hp-=500; p.ft=5; addVFX(p.x,p.y,500); }}); }
            ...
            if(this.r==='gojo' && mD<300 && this.cd<=0) {
                ...
                if(rl<0.33) { this.gAtk=1; ppl.forEach(p=>{if(p.t!==this.t&&Math.hypot(this.x-p.x,this.y-p.y)<150){p.hp-=80; p.ft=5; addVFX(p.x, p.y, 80); let ang=Math.atan2(p.y-this.y,p.x-this.x); p.vx+=Math.cos(ang)*25; p.vy+=Math.sin(ang)*25;}}); }
                else if(rl<0.66) { this.gAtk=2; ppl.forEach(p=>{if(p.t!==this.t&&Math.hypot(this.x-p.x,this.y-p.y)<250){p.hp-=50; p.ft=5; addVFX(p.x, p.y, 50); let ang=Math.atan2(p.y-this.y,p.x-this.x); p.vx-=Math.cos(ang)*20; p.vy-=Math.sin(ang)*20;}}); }
                else { this.gAtk=3; ppl.forEach(p=>{if(p.t!==this.t&&Math.abs(p.y-this.y)<60) { p.hp-=300; p.ft=5; addVFX(p.x, p.y, 300); }}); }
            }
            if(this.r==='goku' && mD<400 && !this.kUse) { this.kUse=true; this.kTmr=60; shakeTime = 30; ppl.forEach(p=>{if(p.t!==this.t&&Math.abs(p.y-this.y)<60) { p.hp-=9999; p.ft=5; addVFX(p.x, p.y, 9999); }}); }
            ...
            if(this.r==='shifter' && this.cd<=0 && mD<150 && !nE.isShifter && nE.r!=='cannon' && !nE.isA) { this.r=nE.r; nE.r='normal'; this.cd=10; nE.ft=10; nE.vx-=Math.cos(a)*15; nE.vy-=Math.sin(a)*15; }
        ...
        this.vx*=0.88; this.vy*=0.88; let spd=Math.hypot(this.vx,this.vy); if(spd>sL){this.vx=(this.vx/spd)*sL; this.vy=(this.vy/spd)*sL;}
        ...
        if(nE && this.r!=='archer' && this.r!=='cannon' && !['gojo','luffy','sukuna','ichigo'].includes(this.r) && this.dT<=0 && this.cd<=0) {
            const d=Math.hypot(this.x-nE.x, this.y-nE.y); let rh=(this.r==='elephant'||this.r==='eren_titan')?70:(this.r==='kurama'?60:40);
            if(d<rh && (wallOpen || this.isA || nE.isA || Math.abs(this.x-nE.x)>30)) {
                
                // CLEARED: Removed massive magic/weapon damage modifier blocks. Clean flat damage!
                let dmg=1.0; if(this.r==='elephant'||this.r==='eren_titan')dmg=8.0; if(this.r==='knight')dmg=2.5; if(this.r==='ninja')dmg=1.5; if(this.r==='goku')dmg=15; if(this.r==='naruto')dmg=5; if(this.r==='kurama')dmg=25; if(this.r==='zoro')dmg=10; 
                ...
                if(this.r==='ninja'){this.vx-=Math.cos(aE)*15; this.vy-=Math.sin(aE)*15;}
            }
        }
    }
}
