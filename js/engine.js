...
// Added new global arrays: particles, dmgTexts, and shakeTime
var ppl = [], projs = [], bldgs = [], particles = [], dmgTexts = [];
var curPwr = 'spawn-normal', wallOpen = false, civOn = true, shakeTime = 0;
...
function animate() {
    ...
    // Draw Particles & Text
    particles.forEach(p=>{p.update();p.draw();}); particles = particles.filter(p=>p.l>0);
    dmgTexts.forEach(d=>{d.update();d.draw();}); dmgTexts = dmgTexts.filter(d=>d.l>0);
